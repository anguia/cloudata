...
CREATE OR REPLACE FUNCTION clean_data(vi_dealdate text) RETURNS void
    AS $$
declare
	v_end_date text;
	v_table_name text;
...
--
-- Name: etl_ip(text); Type: FUNCTION; Schema: public; Owner: aidns
--

CREATE OR REPLACE FUNCTION etl_ip(vi_dealdate text) RETURNS void
    AS $$
declare
...
begin
	...
	select a.DATE_TIME, a.USER_NAME, a.USER_IP, a.AC_IP, stype, err_type, a.DETAIL_INFO, a.USER_AGENT
	from (select  DATE_TIME, USER_NAME, USER_IP, AC_IP, stype, err_type, DETAIL_INFO, USER_AGENT,sum(case when detail_info='用户密码错误' then 1 else 0 end)
	   over (partition by user_name) as num from  TEMP_MONITOR_LOG_2
	) a
	where  num<=50;
...
CCREATE OR REPLACE FUNCTION etl_rpt_cboss_monitor_day(vi_dealdate text)
  RETURNS void AS
$BODY$
declare
