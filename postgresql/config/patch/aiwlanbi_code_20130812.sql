...
begin
	...
	from (
		...
		from temp_cha_err t
		where t.err_type = 4
		and t.date_time >= to_timestamp(vi_dealdate,'yyyy-mm-dd')
		and t.date_time < to_timestamp(v_end_date,'yyyy-mm-dd')
	) a 
	left join (
...
begin
	...
	select odate, case when b.prov_id is null then -1 else b.prov_id end as prov_id
	...
	from (
		select date_trunc('day', date_time) as odate, user_ip, fn_subscription(user_name, v_m_tel, v_nm_tel) as mn_flag, count(1) as p_count
		from TEMP_NORMAL_LOGIN_REQUEST
		group by odate, user_ip, mn_flag
	)a  
	left join SYS_prov_ipseg_info b on a.user_ip between b.start_ip and b.end_ip
	where odate = to_date(vi_dealdate, 'yyyy-mm-dd')
	group by odate, prov_id;
...
begin
	...
	select a.odate, a.prov_id, a.user_ip, max(case when status_code = 200 and b.user_ip is null then 1 else 0 end) as POTENTIAL_USER
...
--
-- Name: public; Type: ACL; Schema: -; Owner: aidns
--
